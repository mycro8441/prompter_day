...
const SliderBlock = styled.div<{ activated: boolean }>`
  ...
  background-image: ${(p) =>
    p.activated &&
    'linear-gradient(#F8F8FB, #F8F8FB), linear-gradient(to bottom, white 0%, #F8F8FD 100%)'};
...
const Create_Result = ({ data }) => {
  ...
  return (
    <Container_Created>
      <p>생성된 플랜</p>
      <h2>텍스트를 클릭하면 수정할 수 있어요</h2>
      <Slider>
        {[...Array(4)].map((v, i: 0 | 1 | 2 | 3) => (
          <SliderBlock
            activated={mode === i}
            onClick={() => setMode(i)}
            key={i}
