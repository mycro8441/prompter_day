...
import { getCalendar, getPlan } from 'src/lib/routes'
...
const Day = styled.div<{ activated: 1 | 0 }>`
  ...
  background-image: ${(p) =>
    p.activated &&
    `linear-gradient(#F6F6FACC, #F6F6FACC), linear-gradient(to bottom, white 0%, #F6F6FACC 100%)`};
...
const CalendarBackground = styled.div`
  ...
  background-image: linear-gradient(#f8f8fd99, #f8f8fd99),
    linear-gradient(to bottom, white 0%, #f8f8fd99 100%);
  background-origin: border-box;
  background-clip: content-box, border-box;
      ...
      {date.map((v, i) => (
        <>
          <WeekDayContainer key={i}>
            <Day activated={getDay(selectedDate) === i ? 1 : 0}>{v}</Day>
          </WeekDayContainer>
        </>
      ))}
    </DayHeader>
...
const Adjust = styled.div`
  ...
  display: flex;
`
const Bay = styled.div<{ slided: boolean }>`
  height: auto;
  width: 100%;
  transform: ${(p) => p.slided && 'translateX(-100%) translateX(-3em)'};
...
const RenderCells = ({ currentMonth, selectedDate, onDateClick }) => {
  ...
  while (day <= endDate) {
    ...
    rows.push(<DaysContainer key={day.getTime()}>{days}</DaysContainer>)
  ...
  return (
    <>
      <Adjust>
        ...
        <Bay slided={slided}>
          <HeaderContainer>
            <p>{format(selectedDate, 'yyyy년 MM월 dd일')}</p>
          </HeaderContainer>
          <Container>
            <CalendarBlur />
            <CalendarContainer>
